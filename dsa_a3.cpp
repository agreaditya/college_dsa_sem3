...
struct node {
  int data;
  node *next; // the next node reference
...
node *insertion_in_beg(node *header) {
  ...
  val = read_data();
  newnode->data = val;
  /*newnode is a pointer to structure. to access its members we use ->
...
void insert_after_value(node *header) {
  node *newnode;
  newnode = new node;
  int val;
  val = read_data();
